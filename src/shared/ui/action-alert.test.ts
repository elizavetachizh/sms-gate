...
import { getActionAlertContent } from "./action-alert-content";

describe("getActionAlertContent", () => {
  it("returns success content for created message", () => {
    expect(
      getActionAlertContent("created", { entity: "message" }),
    ).toMatchObject({
      variant: "success",
      title: "Создано",
  ...
  it("returns success content for updated mailing", () => {
    expect(
      getActionAlertContent("updated", { entity: "mailing" }),
    ).toMatchObject({
      variant: "success",
      title: "Сохранено",
  ...
  it("returns success content for deleted template", () => {
    expect(
      getActionAlertContent("deleted", { entity: "template" }),
    ).toMatchObject({
      variant: "success",
      title: "Удалено",
